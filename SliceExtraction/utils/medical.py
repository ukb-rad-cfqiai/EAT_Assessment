...
from utils.dataReader import (EfficientDataLoader, LandmarkDataset)
...
class MedicalPlayer(gym.Env):
    ...
    def __init__(self, directory=None, viz=False, task=False, files_list=None, landmark_ids=None,
                 screen_dims=(27, 27, 27), history_length=28, multiscale=True,
                 max_num_frames=0, saveGif=False, saveVideo=False, agents=1,
                 oscillations_allowed=4, fixed_spawn=None, logger=None, norm_spacing = True):
        """
        ...
        #self.files = filesListLandmarks(files_list, returnLandmarks, norm_spacing, self.agents)
        if norm_spacing:
            target_spacing = (1,1,1)
        else:
            target_spacing = None
        # sample a new image
        self.dataset = LandmarkDataset(files_list, num_agents=self.agents, target_spacing = target_spacing, num_workers=4)
        self.efficient_loader = EfficientDataLoader(self.dataset, shuffle=False)
    ...
    def new_random_game(self, fixed_spawn=None):
        ...
        # sample a new image
        self._image, self._target_loc, self.filepath, self.spacing, self.landmark_org = next(self.efficient_loader)
        self.filename = [
            os.path.basename(
    ...
    def step(self, act, q_values, isOver):
        ...
        if self.task != 'play' and self.task != 'eval':
            for i in range(self.agents):
                if go_out[i]:
                    self.reward[i] = -1
                else:
        ...
        if self.task != 'play':
            for i in range(self.agents):
                self.cur_dist[i] = calcDistance(self._location[i],
                                                self._target_loc[i],
                                                self.spacing)
        ...
        for i in range(self.agents):
            ...
            info[f"filepath{i}"] = self.filepath[i]
            ...
            info[f"spacing_{i}"] = self.spacing
            info[f"landmark_org_{i}"] = self.landmark_org

        return self._current_state(), self.reward, self.terminal, info

    def getBestLocation(self):
        ''' get best location with best qvalue from last four locations
        stored in history
        '''
        best_locations = []
        for i in range(self.agents):
            last_qvalues_history = self._qvalues_history[i][-10:]
            last_loc_history = self._loc_history[i][-10:]
    ...
    def _current_state(self):
        ...
        for i in range(self.agents):
            ...
            screen[i,
                screen_xmin:screen_xmax,
                screen_ymin:screen_ymax,
                screen_zmin:screen_zmax] = self._image[i][
                    xmin:xmax:self.xscale,
                    ymin:ymax:self.yscale,
                    zmin:zmax:self.zscale]

            ###########################################################
            # update rectangle limits from input image coordinates this is what the network sees
    ...
    def get_plane(self, z=0, agent=0):
        return_value = self._image[agent][:, :, z]
        return return_value
    ...
    def _calc_reward(self, current_loc, next_loc, agent):
        ...
        the target location
        """
        curr_dist = calcDistance(current_loc, self._target_loc[agent], self.spacing)
        next_dist = calcDistance(next_loc, self._target_loc[agent], self.spacing)
    ...
    def display(self, return_rgb_array=False):
        ...
        for i in range(self.agents):
            ...
            self.viewer.display_text(str(self.xscale), color=(204, 204, 0, 255),
                                     x=8,
                                     y=8)

            # -----------------------------------------------------------------
            if self.task != 'play':
                # draw a transparent circle around target point with variable radius based on the difference z-direction
