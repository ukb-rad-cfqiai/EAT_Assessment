...
from utils.expreplay import ReplayMemory
...
class Trainer(object):
    def __init__(self,
        ...
        self.epoch_length       = self.env.dataset.num_files
        ...
        self.dqn                = DQN(self.agents, self.frame_history, logger=logger, collective_rewards=team_reward, attention=attention, lr=lr, scheduler_gamma=scheduler_gamma,                
       		                          scheduler_step_size=scheduler_step_size)
        
        self.dqn.q_network.train(True)
        self.evaluator  = Evaluator(eval_env, self.dqn.q_network, logger, self.agents, steps_per_episode)
        self.logger     = logger
    ...
    def train(self):
        ...
        while episode <= self.max_episodes:
            ...
            for step_num in range(self.steps_per_episode):
                ...
                next_obs, reward, terminal, info = self.env.step(np.copy(acts), q_values, terminal)
            ...
            epoch_distances.append([info['distError_' + str(i)] for i in range(self.agents)])
            ...
            if episode % self.epoch_length == 0:
                self.append_epoch_board(epoch_distances, self.eps, losses, "train", episode)
    ...
    def init_memory(self):
        ...
        pbar = tqdm(desc="Minit_memoryemory buffer", total=self.init_memory_size)
    ...
    def validation_epoch(self, episode):
        ...
        epoch_distances = []
        for k in range(self.eval_env.dataset.num_files):
            self.logger.log(f"eval episode {k}")
            (score, start_dists, q_values, info) = self.evaluator.play_one_episode()
            epoch_distances.append([info['distError_' + str(i)] for i in range(self.agents)])

        val_dists = self.append_epoch_board(epoch_distances, name="eval", episode=episode)
    ...
    def append_episode_board(self, info, score, name="train", episode=0):
        dists  = {str(i): info['distError_' + str(i)] for i in range(self.agents)}
    ...
    def append_epoch_board(self, epoch_dists, eps=0, losses=[], name="train", episode=0):
        ...
        for i in range(self.agents):
            ...
            self.logger.write_to_board(f"{name}/mean_dist", mean_dist_dict, episode)
            min_dist_dict  = {str(i): min(epoch_dists[:, i])}
            self.logger.write_to_board(f"{name}/min_dist", min_dist_dict, episode)
            max_dist_dict  = {str(i): max(epoch_dists[:, i])}
            self.logger.write_to_board(f"{name}/max_dist", max_dist_dict, episode)
